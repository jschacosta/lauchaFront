<template>
  <div>
    <v-card class="elevation-15">
      ...
      <v-data-table
        :headers="headers"
        :items="partidosCalendario"
        :search="search"
        class="elevation-2"
        dense
      >
        <template v-slot:[`item.estado`]="{ item }">
          <v-chip :color="getColor(item.estado)" outlined x-small dark>{{
            item.estado
          }}</v-chip>
        </template>
...
export default {
  ...
  created: function () {
    if (this.pedidoCal === false) {
      ...
      this.axios
        .get(`/matches/match-estado/JUGANDO&POR-JUGAR`)
        .then((res) => {
