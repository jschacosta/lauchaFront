      ...
      <v-card class="elevation-12 general" max-width="600px">
        <div v-for="(item, i) of fechas" :key="i">
          ...
          <v-list dense flat class="pa-0 ma-0">
            <v-list-item-group
              class="pa-0 fondoPartidos"
              v-for="(partido, j) of partidosCalendario"
              :key="j"
            >
        ...
        <v-row>
          <v-list-item-content
            v-if="this.partidosCalendario.length === 0"
            class="fondoPartidos"
          >
            <v-list-item-title class="headline mb-1 d-flex justify-center">
              <v-icon large left color="#2C3A47"> fas fa-beer </v-icon>No hay
              Partidos Agendados</v-list-item-title
            >
          </v-list-item-content>
        </v-row>
      </v-card>
