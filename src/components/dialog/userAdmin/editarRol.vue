    ...
    <v-row justify="center">
        <v-dialog v-model="usuarioEdit.estado" persistent max-width="500px">
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="blue darken-1" text @click="usuarioEdit.estado = false">Cancelar</v-btn>
                    <v-btn color="blue darken-1" text @click="cambiarRol()">Cambiar Rol</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </v-row>
...
export default {
    ...
    methods:{
        ...
        cambiarRol(){
            ...
            this.axios.put(`/admin/user/${id}`,nuevoUsuario,this.config)
            .then(res=>{
