<template>
  <v-dialog v-model="dialogo" persistent max-width="800px">
    <v-stepper v-model="paso" dark dense>
      <!-- ENCABEZADO - PASO 1 -->
      <v-stepper-header
        v-if="(imagen === 'xl' || imagen === 'lg') && porJugar.length < 7"
      >
        <v-stepper-step editable :complete="paso > 1" :step="1"
          >Inicio</v-stepper-step
        >
        <!-- ENCABEZADO - PASOS INTERMEDIOS-->
        <div v-for="(item, i) of porJugar" :key="i">
          <v-stepper-step editable :complete="paso > i + 2" :step="i + 2"
      ...
      <v-stepper-items>
        ...
        <v-stepper-content :step="1">
          <v-card class="mb-12 pa-8" color="#2C3A47">
            <h2 class="ml-5 mb-2 font-weight-light">
              Bienvenido al torneo {{ espacio + espacio }} {{ torneos[0].name }}
            </h2>
            <h4 class="ml-5 font-weight-regular">
              Este será tu carta de vaticinios para los próximos encuentros
              deportivos. Muestra tu jerarquía y sapiencia frente a tus débiles
              adversarios
            </h4>
            <h4 class="mt-5 ml-5 mb-10 font-weight-regular">
          ...
          <v-btn class="mx-3" color="primary" @click="nextStep(1)">
            Iniciar</v-btn
          >
        </v-stepper-content>
        ...
        <div v-for="(partido, j) of porJugar" :key="j">
          <v-stepper-content :step="j + 2" class="ma-0 pa-0 pa-3">
            ...
            >
              <v-row class="ma-0 pa-0">
                <v-btn
                  class="mx-2"
                  color="primary"
                  v-if="j + 2 === porJugar.length + 1"
                  @click="nextStep(j + 2)"
                >
                  <v-icon left>keyboard_arrow_left</v-icon> Inicio</v-btn
                >
            ...
            <v-card class="mb-12" color="#2C3A47">
              ...
              <v-btn class="ml-2" color="primary" x-small @click="uncheck(j)"
  ...
  methods: {
    ...
    enviar() {
      ...
      if (index === -1) {
        ...
        this.axios
          .put(`/users/user-torneo`, paraToken)
          .then((res) => {
