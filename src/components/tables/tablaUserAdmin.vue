    ...
    <v-row>
      <v-col cols="12" md="6">
        <div class="text-center">
          <v-btn dark ml-auto color="warning" @click="getUsers()"
            >Obtener Usuarios</v-btn
          >
        </div>
      </v-col>
      <v-col Cols="12" md="6">
        <div class="text-center">
          <v-btn dark ml-auto color="green accent-5" @click="createUser()"
...
export default {
  ...
  methods: {
    ...
    getUsers() {
      ...
      this.axios
        .get(`/users/admin/users`, this.config)
        .then((res) => {
          let users = res.data;
    ...
    deleteUser(user) {
      const info = {
        ...
        cuerpo:
          "Al eliminar, toda la información del usuario será borrada de manera permanente",
        boton: "borrarUsuario",
        datos: [],
      };
      info.datos = user;
