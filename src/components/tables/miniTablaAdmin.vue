<template>
    ...
    <p  class="text-center text-caption"  v-if="imagen==='xs'">Voltea el telefono para más detalles</p>
          <v-list-item-group class="pa-0 ma-0 text-center">
            <v-list-item-content class="pa-0 ma-0 ">
              <v-row class="text-center ma-0 pa-0 text-caption ">
                ...
                <v-col cols="2" class="ancho" v-if="imagen!='xs' && imagen!='sm' && imagen!='md'">MARC EXACTO</v-col>
                <v-col cols="2" class="ancho" v-if="imagen==='sm' || imagen==='md'">M EXACTO</v-col>
                <v-col cols="2" class="ancho" v-if="imagen==='xs' ">ME</v-col>
                <v-col cols="2" class="ancho"  v-if="imagen!='xs' && imagen!='sm' && imagen!='md'">PRONOST ESP</v-col>
                <v-col cols="2" class="ancho" v-if="imagen==='sm' || imagen==='md'">PRON ESP</v-col>
            ...
            <v-list-item-content v-for="(item2,j) of item" :key="j" class="pa-0 ma-0" background-color="primary">
              ...
              <v-divider vertical v-if=" imagen!='xs'"></v-divider>
                ...
                <v-col v-if="item.reduce((a, b) => a + b, 0)===0 && imagen!='xs' " cols="2" class="ancho" ><v-chip :x-small="imagen==='xs'?true:false" color="grey white--text" class="px-5">0</v-chip></v-col>
                ...
                <v-col v-if="item.reduce((a, b) => a + b, 0)===0 && imagen==='xs'" cols="2" class="ancho gris">
      
                 -{{item.reduce((a, b) => a + b, 0)}}00 </v-col>
